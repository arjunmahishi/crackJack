...
def upload(request):
	...
	key_dict = {}
	for key in key_data:
		key_dict[key.strip().split('~~')[0].replace(' ', '')] = key.strip().split('~~')[1].replace(' ', '')
	...
	if request.method == 'POST' and request.FILES['myfile']:
		...
		if team_name in key_dict.keys() and key_dict[team_name] == team_pass:
			print "going in!"
			fs = FileSystemStorage()
			filename = fs.save('uploadedFiles/' + team_name + '/' + myfile.name, myfile)
			uploaded_file_url = fs.url(filename)
			msg = 'Uploaded successfully !'
		elif team_name == '' and team_pass =='':
			msg = ''
		else:
